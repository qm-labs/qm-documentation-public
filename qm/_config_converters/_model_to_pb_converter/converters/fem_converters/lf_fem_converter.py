...
class LfFemToPbConverter(FemToPbConverter[FemDataOctoDac, QuaConfig.OctoDacFemDec]):
    ...
    def update_sampling_rate_enum(
        ...
        Args:
            ...
            data: The analog output port model holding the ``sampling_rate`` and ``upsampling_mode``.
        """
        sampling_rate = (
    ...
    def deconvert(self, output_data: QuaConfig.OctoDacFemDec, fem: LfFem) -> FemDataOctoDac:  # type: ignore[override]
        ...
        Args:
            output_data: The protobuf LF FEM declaration to convert back into a model.
            fem: The FEM that the deconverted ports belong to.

        Returns:
            The LF FEM data model (analog/digital inputs and outputs) reconstructed from the protobuf.
        """
