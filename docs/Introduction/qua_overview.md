...
The Quantum Universal Abstraction (QUA) programming language is a high-level programming language designed for pulse level control of quantum devices.
The source code for the Pythonic implementation (DSL) of QUA can be found [here](https://github.com/qm-labs/qm-qua-sdk-public).
...
In addition to the specification of which pulses are played, it also specifies when they should be played through both explicit and implicit statements and dependency constructs. 
Thus, a QUA program also defines exactly the timing in which pulses are played, down to the single sample level.

The pulse syntax defines an implicit pulse dependency, which determines the order of pulse execution.
...
or manipulate the pulse according to the element's properties defined in the logical configuration.

## Analog Waveform Manipulations

### Single Input Element

If the considered element has a single input, the pulse sent to it must be defined with a single waveform.
For example, the logical configuration section would look like:
...
If the considered element is connected to a single MW output (MW-FEM), the pulse sent to it must be defined with a two waveform.
For example, the logical configuration section would look like:
...
A pulse sent to such an element must be defined with two waveforms. For example:
...
Where `A` can either be a matrix or a Scalar (a [Single Input Element](#single-input-element) can only accept a scalar).
If a scalar `A` is supplied to a [MWInput Element](#mwinput-element) or a [Mixed Inputs Element](#mixed-inputs-element), then it is multiplied by the identity matrix.
...
The signal going from the OPX has some propagation delay as it travels down the wire and towards the switch. 
The returning signal will be further delayed until it reaches the second switch.
...
Because they are associated with the connections to the device (wire lengths, resonator ring up times, etc).
Setting up these parameters in QUA is done by setting the values as part of the element configuration.
Each such digital input is called a **digital route**.

!!! Important
    There is a maximal number of digital routes, which depends on the specific version. 
    That is, the total number of digital inputs that can be defined for all elements in a program.
    In addition, recording an ADC trace also uses a digital route.
    Attempting to declare more routes than available, will result in a failure to allocate resources.

!!! Note
    There is an intrinsic delay for the analog channels with respect to the digital channel.

### Configuring a digital pulse

To define a digital input of an element the configuration must have the following three properties: `port`,
...
A pulse played to an element with a digital input, can include a single digital marker which points to a single digital waveform.
For example:
...
A length of 0 indicates that the corresponding value is to be played for the remaining duration of the pulse.
...
- Plays the digital waveform to the digital output of the OPX as defined in the associated element.
...
Where 136 ns is an example for the intrinsic delay discussed above.

### Digital markers and element readout

The output of an element can be read by the OPX. Returning to the example we considered above, the same effects
...
In the configuration, the element's frequency is defined in units of Hz and with a type of `double`, allowing sub-Hz resolution.
From QUA, Sub-Hz resolution can be achieved by setting the `Units` parameter to the desired resolution.
For example:
...
As seen in this example, when an element has outputs, two additional properties must be defined:
...
3. Demodulate the sampled data with a frequency `intermediate_frequency`, defined in the configuration of the element,
   ...
   logical configuration, and put the result in the indicated variable. The OPX can perform multiple demodulations and
...
Further explanation can be found in the [dual demodulation](../Guides/demod.md#dual-demodulation) section.
